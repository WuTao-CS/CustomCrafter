...
from lvdm.modules.attention import BasicTransformerBlock, CrossAttention
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if self.frame_cond:
            ...
            print("---training for %d-frame conditoning T2V"%(self.frame_cond))
        else:
            self.cond_mask = None
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (batch_idx+1) % self.log_every_t == 0:
            print(f"batch:{batch_idx}|epoch:{self.current_epoch} [globalstep:{self.global_step}]: loss={loss}")
        return loss
    ...
    #     }
    #     batch_logs = self.log_images(batch, **log_images_kwargs)
    #     print(f"eval-batch:{batch_idx} [globalstep:{self.global_step}]: loss={loss}")

    #     ## move to CPU and save images
    #     batch_logs = prepare_to_log(batch_logs)
    #     torch.cuda.empty_cache()
    #     filename = "idx{}_rank{}".format(batch_idx, self.global_rank)
    #     log_local(batch_logs=batch_logs, save_dir=self.val_dir, filename=filename, save_fps=10)
    #     return loss
    ...
    #     print(f"@validating at [epoch: {self.current_epoch} | global_step: {self.global_step}]...")

    # @torch.no_grad()
    # def validation_epoch_end(self, outputs):
    #     ## collect real and fake
    #     #real_list, fake = load_from_valepoch(self.val_dir)
    #     torch.cuda.empty_cache()
    #     if dist.get_rank() == 0:
    #         #real = real.cpu().add_(1).mul(127.5).numpy().astype(np.uint8).transpose(0,2,3,4,1)
    #         #fake = fake.cpu().add_(1).mul(127.5).numpy().astype(np.uint8).transpose(0,2,3,4,1)
    #         #n_samples = fake.shape[0]
    #         torch.cuda.empty_cache()
    #         fvd, kvd, n_samples = 0.000, 0.000, 2048
    #         print('metric calculating over %d samples'%n_samples)
    #         #fvd, kvd, n_samples = cal_fvd(real, fake, device=self.device, batch_size=1, n_sample=n_samples)
    #         val_metrics= {
    ...
    def log_images(self, batch, sample=True, ddim_steps=200, ddim_eta=1., plot_denoise_rows=False, \
                    unconditional_guidance_scale=15.0, **kwargs):
    ...
    def configure_optimizers(self):
        ...
        if self.empty_params_only and hasattr(self, "empty_paras"):
            ...
            print(f"@Training [{len(params)}] Empty Paramters ONLY.")
        else:
            params = list(self.model.parameters())
            print(f"@Training [{len(params)}] Full Paramters.")
        ...
        if self.freeze_model == 'crossattn-kv':
            ...
            for x in self.model.diffusion_model.named_parameters():
                if 'transformer_blocks' not in x[0]:
                    ...
                    x[1].requires_grad = True
        elif self.freeze_model == 'crossattn-kv-ffn':
            if 'transformer_blocks' not in x[0]:
                    x[1].requires_grad = False
            elif 'init_attn' in x[0]:
                    x[1].requires_grad = False
            elif not ('attn2.to_k' in x[0] or 'attn2.to_v' or '.ff.' in x[0]):
                x[1].requires_grad = False
            else:
                x[1].requires_grad = True
            ...
            for x in self.model.diffusion_model.named_parameters():
                if 'transformer_blocks' not in x[0]:
                    ...
                    x[1].requires_grad = False
                elif not ('attn2.to_q' in x[0] or 'attn2.to_k' in x[0] or 'attn2.to_v' or '.ff.' in x[0]):
                    x[1].requires_grad = False
                else:
                    x[1].requires_grad = True
        elif self.freeze_model == 'updateall':
            for x in self.model.diffusion_model.named_parameters():
                x[1].requires_grad = True
        elif self.freeze_model == 'frozen':
            for x in self.model.diffusion_model.named_parameters():
                x[1].requires_grad = False
        else:
            raise NotImplementedError
