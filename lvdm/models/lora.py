...
def _find_modules_v2(
    ...
    module_child_name: Optional[List[str]] = None,
    ...
    if type(ancestor_class) is not set:
        ancestor_class = set(ancestor_class) # {'FeedForward', 'BasicTransformerBlock', 'CrossAttention'}
        # print(ancestor_class)
    if type(module_child_name) is not list and module_child_name is not None:
        module_child_name = list(module_child_name)
    ...
    for ancestor in ancestors:
        for fullname, module in ancestor.named_children():
            tmp=[name for name,p in ancestor.named_children()]
            # print(f'ancestor child modules name={tmp}')
            if any([isinstance(module, _class) for _class in search_class]):
                # print(f'find a child is nn linear, childname={fullname}')
                ...
                *path, name = fullname.split(".")
                ...
                if exclude_children_of and any(
                    ...
                    continue
                # Skip this linear if it is not target child layer
                if module_child_name is not None and name not in module_child_name:
                    continue
...
def inject_trainable_lora(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if verbose:
            print(f"LoRA Injection : injecting lora into: module={_module.__class__.__name__}, name={name}, _child_module={_child_module.__class__.__name__}")
...
def net_load_lora(net, checkpoint_path, alpha=1.0, remove=False, 
    ...
    for key in state_dict:
        ...
        for item in pair_keys:
            visited.append(item)
    # clip_sd.pop('cond_stage_model.token_embedding.weight')
    if load_text_encoder:
        modelsd = net.state_dict()
        for k, v in clip_sd.items():
            modelsd[k] = v
        
        net.load_state_dict(modelsd, strict=True)
