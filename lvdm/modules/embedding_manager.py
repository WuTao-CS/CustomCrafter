...
class EmbeddingManager(nn.Module):
    def __init__(
        ...
        if isinstance(embedder, lvdm.modules.encoders.condition.FrozenCLIPEmbedder):
            get_token_for_string = partial(get_clip_token_for_string, embedder.tokenizer)
            get_embedding_for_tkn = partial(get_embedding_for_clip_token, embedder.transformer.text_model.embeddings)
            token_dim = 768
        elif isinstance(embedder, lvdm.modules.encoders.condition.FrozenOpenCLIPEmbedder):
        ...
        for idx, placeholder_string in enumerate(placeholder_strings):
            ...
            self.string_to_token_dict[placeholder_string] = token.item()
