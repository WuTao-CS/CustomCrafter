...
class IDAdapter(nn.Module):
    def __init__(
        self, in_features, out_features, bias=False, r=4, scale=1.0
    ):
        super().__init__()
    ...
    def forward(self, input):
        return (
            self.linear(input)
            + self.lora_up(self.selector(self.lora_down(input)))
            * self.scale
        )
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True,
                disable_self_attn=False, attention_cls=None, img_cross_attention=False, ip_adapter=False):
        super().__init__()
        ...
        if self.ip_adapter:
            self.adapter = IDAdapter(dim, dim, r=4, scale=1.0)
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 use_checkpoint=True, disable_self_attn=False, use_linear=False, img_cross_attention=False, ip_adapter=False):
        super().__init__()
        ...
        self.transformer_blocks = nn.ModuleList([
            BasicTransformerBlock(
                ...
                ip_adapter=ip_adapter) for d in range(depth)
        ])
        if not use_linear:
