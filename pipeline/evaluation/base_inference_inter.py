        ...
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            for inter_sample in inter_samples:
                for nn, sample in enumerate(inter_sample):
                    for mm,sub_video in enumerate(sample):
                        sub_video = sub_video.unsqueeze(0)
                        prompt = prompts[nn]
                        filename = "%04d_randk%d"%(idx*args.bs+nn+gpu_no*len(prompt_list_rank), gpu_no)+"_inter{}_{}".format(nn,mm)
                        save_results(prompt, sub_video, None, filename, realdir, fakedir, fps=8)
    else:
        ## dataset settting
        try:
