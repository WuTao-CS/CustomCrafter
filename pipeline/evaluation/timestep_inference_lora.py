...
def save_results(prompt, samples, inputs, filename, realdir, fakedir, fps=10):
    ...
    for idx, video in enumerate(videos):
        ...
        if n>5:
            ...
            num_row = int(n)
        frame_grids = [torchvision.utils.make_grid(framesheet, nrow=num_row) for framesheet in video] #[3, 1*h, n*w]
...
def inference_prompt(model, prompts, pretrain_path, ckpt_path, mid_step, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                unconditional_guidance_scale=1.0, log_every_t=1, save_freq=5, begin_scale=0.4, mid_scale=0.8, downsample=False, res_step=0,neg=False, **kwargs):
    ...
    if unconditional_guidance_scale != 1.0:
        if neg:
            prompts = batch_size * ["blurry background, blurry picture, Stationary, blurry, low-motion, deformed, lowres, bad anatomy, text, error, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry"]
        else:
            prompts = batch_size * [""]
    ...
    batch_variants = []
    inter_batch_variants = []
    inter_images = []
    for _ in range(n_samples):
        model.reset_lora_scale(new_scale=begin_scale)
        model.eval()
        episodes = None
        cond_mask = None
        ...
        x_t = intermediates['x_inter'][mid_step-1]
        if downsample:
            print("##############")
            print(x_t.shape)
            N = 2
            # x_t = model.decode_first_stage(x_t)
            # print(x_t.shape)
            downsampled_size = (x_t.size(2), x_t.size(3) // N, x_t.size(4) // N)
            downsampled_x = F.interpolate(x_t, size=downsampled_size, mode="nearest")
            print(downsampled_x.shape)
            x_t = F.interpolate(downsampled_x, size=(x_t.size(2),x_t.size(3), x_t.size(4)), mode="nearest")
            # x_t = model.encode_first_stage(x_t)
            # downsampled_size = (x_t.size(2), x_t.size(3) // N, x_t.size(4) // N)
            

        model.reset_lora_scale(new_scale=mid_scale)
        model.eval()
        cond_mask = None
        for idx, cond in enumerate(cond_episodes):
            kwargs.update({"clean_cond": False})
            samples, intermediates = ddim_sampler.sample(S=ddim_steps,
                                            ...
                                            shape=noise_shape[1:],
                                            x_T=x_t,
                                            timesteps=ddim_steps-mid_step+res_step+1,
                                            verbose=True,
                                            unconditional_guidance_scale=unconditional_guidance_scale,
                                            unconditional_conditioning=uc,
                                            eta=ddim_eta,
        ...
        cnt = 0
        for inter in intermediates['x_inter']:
            cnt+=1
            if cnt % save_freq == 0:
                inter_images.append(model.decode_first_stage(inter).cpu())
                inter_batch_variants.append(torch.stack(inter_images).permute(1, 0, 2, 3, 4, 5))
    ## variants, batch, c, t, h, w
    batch_variants = torch.stack(batch_variants)
    all_inter_batch_variants = []
    for inter in inter_batch_variants:
        all_inter_batch_variants.append(inter_batch_variants)
    return batch_variants.permute(1, 0, 2, 3, 4, 5), inter_batch_variants
...
def run_inference(args, gpu_num, gpu_no):
    ...
    if args.lora and args.begin_scale is not None:
        model_config['params']['lora_scale'] = args.begin_scale
    ...
    # model = load_model_checkpoint(model, args.pretrain_path)
    model.eval()
    ...
    if args.prompt_file:
        ...
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            batch_samples,inter_samples = inference_prompt(model, prompts, args.ckpt_path, args.ckpt_path, args.mid_step, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
                                        args.unconditional_guidance_scale,
                                        cond_tau=args.cond_tau, target_size=args.target_size,begin_scale=args.begin_scale, mid_scale=args.mid_scale, downsample=args.downsample, res_step=args.res_step,neg=args.neg)
            ...
            # for inter_sample in inter_samples:
            #     for nn, sample in enumerate(inter_sample):
            #         prompt = prompts[nn]
            #         filename = "%04d_randk%d"%(idx*args.bs+nn+gpu_no*len(prompt_list_rank), gpu_no)+"_inter{}".format(nn)
            #         save_results(prompt, sample, None, filename, realdir, fakedir, fps=8)
    else:
        ## dataset settting
        try:
        ...
        for idx, batch in tqdm(enumerate(dataloader_rank), desc='Sample Batch'):
            ...
            batch_samples = inference_prompt(model, prompts, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
                                        args.unconditional_guidance_scale, 
                                        cond_tau=args.cond_tau, target_size=args.target_size,downsample=args.downsample, res_step=args.res_step,neg = args.neg)
            ## save each example individually
...
def get_parser():
    ...
    # parser.add_argument("--pretrain_path", type=str, default=None, help="pretrain_checkpoint path")
    ...
    parser.add_argument("--mid_step", type=int, default=20, help="")
    parser.add_argument("--res_step", type=int, default=0, help="")
    parser.add_argument("--lora", type=bool, default=False, help="is use lora",)
    parser.add_argument("--begin_scale", type=float, default=0.4, help="lora_scale",)
    parser.add_argument("--mid_scale", type=float, default=0.8, help="lora_scale",)
    parser.add_argument("--downsample", type=bool, default=False, help="downsample mid",)
    parser.add_argument("--pretrain", type=str, default='checkpoints/videocrafter2/model.ckpt', help="pretrain model path")
    parser.add_argument("--neg", type=bool, default=False, help="is use lora",)
    return parser
