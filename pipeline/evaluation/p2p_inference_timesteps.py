...
def inference_prompt(model, prompts, noise_shape, cross_attention_kwargs, n_samples=1, ddim_steps=50, ddim_eta=1., \
                unconditional_guidance_scale=1.0, mid_step=5, log_every_t=1, **kwargs):
    ...
    for _ in range(n_samples):
        ...
        for idx, cond in enumerate(cond_episodes):
            ...
            model.reset_lora_scale(new_scale=1.0)
            model.eval()
            cond_mask = None
            for idx, cond in enumerate(cond_episodes):
                kwargs.update({"clean_cond": False})
...
# , The full picture.
def run_inference(args, gpu_num, gpu_no, prompt_list=['a new teddybear is playing guitar. Complete picture','a <new1> teddybear is playing guitar. Complete picture'],
