...
def inference_prompt(model, prompts,pretrain_path, ckpt_path, mid_step, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                unconditional_guidance_scale=1.0, log_every_t=1, save_freq=5,**kwargs):
    ...
    for _ in range(n_samples):
        model = load_model_checkpoint(model, pretrain_path)
        model.eval()
        episodes = None
        cond_mask = None
        ...
        for idx, cond in enumerate(cond_episodes):
            ...
            samples, intermediates = ddim_sampler.sample(S=ddim_steps,
                                            ...
                                            shape=noise_shape[1:],
                                            x_T=x_t,
                                            timesteps=ddim_steps-mid_step+1,
                                            verbose=True,
                                            unconditional_guidance_scale=unconditional_guidance_scale,
                                            unconditional_conditioning=uc,
                                            eta=ddim_eta,
        ...
        for inter in intermediates['x_inter']:
            cnt+=1
            if cnt % save_freq == 0:
                inter_images.append(model.decode_first_stage_2DAE(inter).cpu())
...
def run_inference(args, gpu_num, gpu_no):
    ...
    if args.prompt_file:
        ...
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            batch_samples,inter_samples = inference_prompt(model, prompts,args.pretrain_path, args.ckpt_path, args.mid_step, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
