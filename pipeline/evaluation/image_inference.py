...
def inference_prompt(model, prompts, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                unconditional_guidance_scale=1.0, image_feature=None, **kwargs):
    ...
    for episode in range(n_episodes):
        ...
        batch_cond = model.get_learned_conditioning(batch_cond, image_feature=image_feature)
...
def run_inference(args, gpu_num, gpu_no):
    ...
    image_feature = torch.load(args.img_feature)[0]
    print(image_feature)
    start = time.time()  
    if args.prompt_file:
        ## prompt file setting
        ...
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            batch_samples = inference_prompt(model, prompts, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
                                        ...
                                        cond_tau=args.cond_tau, target_size=args.target_size, image_feature=image_feature)
